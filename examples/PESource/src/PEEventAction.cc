...
PEGasHitsCollection* 
PEEventAction::GetGasHitsCollection(G4int hcID,
                                    const G4Event* event) const
{
  auto hitsCollection 
    = static_cast<PEGasHitsCollection*>(
        event->GetHCofThisEvent()->GetHC(hcID));
  
  if ( ! hitsCollection ) {
    G4ExceptionDescription msg;
    msg << "Cannot access hitsCollection ID " << hcID; 
    G4Exception("PEEventAction::GetGasHitsCollection()",
...
void PEEventAction::BeginOfEventAction(const G4Event*
                                         /*event*/)
{ ; }

void PEEventAction::EndOfEventAction(const G4Event* event)
{
